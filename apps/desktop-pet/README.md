...
自动行走的默认速度与节奏在 `src/movement.ts` 的 `DEFAULT_MOVEMENT_OPTIONS` 中（默认 `48` 逻辑像素/秒、无起步缓动）。每只宠物也可以在自己的 `pet.json` 里单独覆盖：
...
`rampMs` 为起步/转向的加速时间（毫秒）；未配置的宠物保持原有的瞬时起步行为。移动距离基于真实时间差（wall-clock）计算并做亚像素累积，速度在 60Hz、90Hz、120Hz 以及更高刷新率的显示器上保持一致，窗口隐藏、系统休眠或拖动结束后也不会产生一次性的跳跃位移。
