Create Codex pet row `running-right` for `niuniu`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: the exact black-and-white cat in the reference: dominant black crown, narrow white forehead blaze, yellow-green eyes, pink nose, white chest and legs, black side patches, rounded compact body, subtle cyan pixel-edge accent. Preserve silhouette, face, palette, material, proportions, markings, and props.
